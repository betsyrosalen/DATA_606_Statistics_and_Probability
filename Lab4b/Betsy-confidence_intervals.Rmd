...
"typical" to mean.
    ...
    The sample distribution has a slight right skew with most houses falling around the 1500 mark, and a range from 520 to 2592.  A 'typical' home in this sample is about 1361 square feet which is the median of the sample.  I chose the median since the distribution is skewed and so the median is a better estimate of the 'typical' observation although in this case the mean and median are not too far apart so the mean of 1430 would also be a good estimate of the 'typical' home size.
...
Would you expect it to be similar? Why or why not?
    ...
    I would not expect another student's sample to be identical to mine, but I would expect it to be similar.  No two samples if taken at random would have identical statistics or distributions, but since most samples (if taken at random, and of sufficient size) would be good approximations of the population, I would expect them to be similar to each other.
...
distributed and have standard error $\frac{s}{\sqrt{n}}$. What conditions must be 
met for this to be true?

    - The sample observations must be independent.  
    - The sample size must be at least 30 but not larger than 10% of the population. 
    - The population distribution cannot be strongly skewed.

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

    '95% confidence' means that we are 95% confident that our population parameter falls within the range specified by the confidence interval.
...
interval capture this value? 
    ...
    Yes, my confidence interval does capture the true average size of houses in Ames.
...
population mean.
    ...
    I would expect 95% of the confidence intervals to capture the true mean since we created our confidence interval at 95% by using $z^\star = 1.96$.  
...
-   Using the following function (which was downloaded with the data set), 
    ...
    47 out of 50 samples or 94% of my confidence intervals captured the population mean.  This is not exactly equal to the 95% confidence interval but we would not expect it to be exactly equal to 95% since we are taking random samples and the samples are of a relatively small size compared to the size of the total population which has 2930 observations.  And in fact it would be impossible to get exactly 95% accuracy with only 50 samples.  No matter how many are accurate with 50 samples, you have to double the number to get the percent that are accurate, so the percent will always be an even number.

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    
    For a 90% confidence interval you would use $z^\star = 1.65$
    for a 99% confidence interval you would use $z^\star = 2.58$
...
-   Calculate 50 confidence intervals at the confidence level you chose in the 
    ...
    My 90% confidence interval plot returned exactly 90% accurate confidence intervals and my 99% confidence intervals are 98% accurate.  
    
    ```{r ci90, eval=TRUE}
lower_vector90 <- samp_mean - 1.65 * samp_sd / sqrt(n) 
upper_vector90 <- samp_mean + 1.65 * samp_sd / sqrt(n)
