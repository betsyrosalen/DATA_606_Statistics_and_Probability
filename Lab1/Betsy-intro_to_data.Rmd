...
```
    ...
    exerany   | nominal categorical
    hlthplan  | nominal categorical
    smoke100  | nominal categorical
    height    | discrete numerical
    weight    | discrete numerical
    wtdesire  | discrete numerical
...
```
    ...
    ##### 23.285% of the sample reports being in excellent health.
...
3.  What does the mosaic plot reveal about smoking habits and gender?
    ...
    ##### The mosaic plot reveals that the proportion of men who have smoked at least 100 cigarettes in their lifetiome is greater than the proportion of women who have.  
...
5.  What does this box plot show? Pick another categorical variable from the 
    ...
    ```{r}
boxplot(bmi ~ cdc$exerany, horizontal=TRUE)
```
...
```
    ...
    ##### There's a string linear corelation between weight and desired weight.  At the lower end of the range, the weight and desired weight are closer to each other on average,  but as the actual weight gets larger, the corresponding average desired weight dips below the actual weight.  
...
-   Let's consider a new variable: the difference between desired weight 
    ...
    ```{r}
# Remove the two outliers who say they want to gain hundreds of pounds first
clean <- subset(cdc, wtdesire < 400)
wdiff <- clean$weight - clean$wtdesire
head(wdiff, 100)
```
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    `wdiff` is an vector of type int.  If the value is 0 it means the person is happy with the weight they are and does not want to change their weight at all, in other words, their weight and desired weight are exactly the same.  If `wdiff` is positive it means that the person would like to lose that number of pounds.  If `wdiff` is negative it means that the person would like to gain that number of pounds. 
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    `wdiff` is a slightly right sewed distribution with the majority of values at or around zero and a larger tail in the positive direction.  50% of people want to lose between 0 and 21 pounds.  25% of people want to gain weight with a maximum desired weight gain of 110 pounds but the vast majority under 50 pounds.  The last 25% want to lose anywhere from 21 to 300 pounds with the vast majority of those between 21 and 100 pounds.  Because most people are at or close to their desired weight we can conclude that most people are happy with their current weight. 
...
-   Using numerical summaries and a side-by-side box plot, determine if men tend
    ...
    ```{r}
men <- subset(clean, gender == 'm')
women <- subset(clean, gender == 'f')

m_wdiff <- men$weight - men$wtdesire
f_wdiff <- women$weight - women$wtdesire
...
```
    ...
    ```{r}
boxplot(m_wdiff, f_wdiff, horizontal = TRUE)
```

    On average women desire a larger weight loss and much fewer want to gain weight.  
...
s <- sd(cdc$weight)
s
prop.table(table(cdc$weight > (m-s) & cdc$weight < (m+s)))
```

    70.76% of people are within one standard deviation of the mean weight of the sample.
