...
1.  Describe this population distribution.
    ...
    Based on our summary statistics and histogram the above ground living area of the house in square feet seems to be a nearly normal distribution with a slight right skew.  The mean being a bit larger than the median would support the conclusion that there is a right skew.  Also the max of 5642 and min of 334 seem likely to be outliers.  The full range of the data is 5308 sq ft, but 50% of the homes fall in betweeen 1126 and 1743 sq ft, an interquartile range of 617 sq ft, which is considerably smaller than the full range of the data.  
...
2.  Describe the distribution of this sample. How does it compare to the 
    ...
    Our sample has a much smaller range (2076 vs. 5308 in the population) but the interquartile range, mean, and median are very similar to the corresponding population statistics.  The IQR is 653 vs. 617 for the population, the median is 1457 vs. 1442 in the population and the mean is 1540 vs 1500 in the population.  So our sample statistics are mostly good estimates of our population parameters.  The distribution looks like it may be nearly normal with a slight right skew like the population, but it is hard to tell with so few observations in each bin of the histogram.  
...
3.  Take a second sample, also of size 50, and call it `samp2`.  How does the 
    ...
    Our second sample has a mean of 1470, which is slightly smaller than our population mean of 1500 as opposed to our first sample which had a mean slightly larger than our population mean.  If we took two more samples of size 100 and then 1000 I would expect that as our sample size got larger our sample mean would get closer to the population mean, so the sample of 1000 would provide the most accurate estimate of the population mean.
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    There are 5000 elements in sample_means50, each one representing the mean area of a random sample of 50 homes from our population.  The sampling distribution looks normal with it's center at about 1500 which is our population mean.
    
    If we took 50,000 sample means, I would expect the distribution to remain normal with it's center at our population mean, but it would get taller and skinnier in shape.  
    
    The example below looks the same only because it's y-axis has been changed to represent three times as many observations in a histogram of the same height.
...
5.  To make sure you understand what you've done in this loop, try running a 
    ...
    There are exactly 100 elements in sample_means_small, and each one represents a mean area of a sample of size 50 taken from our population.
...
6.  When the sample size is larger, what happens to the center?  What about the spread?
    ...
    When the sample size is larger, the spread of the sampling distribution gets smaller, but the center remains at the population mean.
...
-   Take a random sample of size 50 from `price`. Using this sample, what is 
    ...
    Our best point estimate of the population mean from one sample of 50 is the sample mean, which in this case is $165,990.40.
...
-   Since you have access to the population, simulate the sampling 
    ...
    The sampling distibution of the mean home price of 5000 random samples of size 50 is a normal distrtibution with it's center at about 180,000 and a range of about 140,000 to 220,000.
    
    I would guess the mean home price of the population to be about $180,000 based on the histogram.  The actual population mean is $180,796.10.
...
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    The sampling distibution of the mean home price of 5000 random samples of size 150 is a normal distrtibution with it's center at about 180,000 and a range of about 160,000 to 210,000.  The main differences are that this distribution has a smaller range than my sampling distribution at a sample size of 50 and is taller in the middlw with about 1500 values in teh middle two bars vs. 850 in the middle two bars with the smaller sample size.
    
    I would still guess the mean home price of the population to be about $180,000 based on the histogram.
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    The distribution of means at sample size 150 has the smaller spread.  If
    we're concerned with making estimates that are more often close to the 
    true value, would we prefer a distribution with a smaller spread since this means that the average distance from the sample mean to the population mean is smaller. In other words all of the sample means are closer on average to our population mean than in a sampling distribution with a larger spread. 
