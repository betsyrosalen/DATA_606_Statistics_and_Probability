...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    A streak length of one means that he made exactly one basket before the next one he missed.  There is exactly one hit, and one miss in a streak of length one.  Additional misses at the end of a prior streak are recorded as streaks of length zero.  
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    Kobe's typical streak length was zero and longest streak length was 4.  He had about 40 streaks of length zero and progressively less at greater streak lengths.  Streaks of length 2 and 3 are about the same number and the smallest quantity at length 4 has about 1 streak at that length.
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    ...
    As expected approximately 20% of the flips came up heads in my simulation.
    
```{r table-sim-unfair-coin, eval=TRUE}
sim_unfair_coin
...
1. Describe the distribution of streak lengths. What is the typical streak 
    ...
    The Sim's typical streak length was zero like Kobe's.  The sim had about the same number of streaks of length zero and progressively less at greater streak lengths just like Kobe's.  The smallest quantities are at the longest streak lengths with about 1 streak at that length.
...
2. If you were to run the simulation of the independent shooter a second time, 
    ...
    If I were to run the simulation again I would expect it to be similar, but not exaxctly the same. Since it is a random simulation with the same probability as the first time, the results should be similar each time it is run and similar to Kobe's distribution as well.
...
3. How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    Kobe Bryant's distribution of streak lengths is very similar to the simulation suggesting that the 'hot hand' phenomenon has no factual basis and that longer streaks of baskets are due to chance alone.  If the 'hot hands' theory was true then we would expect there to be more streaks of longer length in Kobe's distribution.  In fact the opposite happened in our first simulation.  We would expect additional simulations would also be similar, but could be more or less.